...
from safe.engine.polygon import separate_points_by_polygon, clip_lines_by_polygon
from safe.engine.polygon import is_inside_polygon
from safe.engine.numerics import cdf, erf, ensure_numeric
...
from safe.storage.utilities_test import TESTDATA
...
from safe.engine.impact_functions_for_testing import NEXIS_building_impact_model
...
class Test_Engine(unittest.TestCase):
    ...
    def test_earthquake_fatality_estimation_allen(self):
        ...
        hazard_filename = '%s/earthquake.tif' % TESTDATA
        exposure_filename = '%s/population.tif' % TESTDATA
    ...
    def test_jakarta_flood_study(self):
        ...
        population = 'population_city.asc'
        plugin_name = 'HKVtest'
    ...
    def test_earthquake_damage_schools(self):
        ...
        for mmi_filename in ['hazmap.asc',
                             'earthquake.tif',  # NaN's
                             ]:

            # Name file names for hazard level and exposure
            hazard_filename = '%s/%s' % (TESTDATA, mmi_filename)
            exposure_filename = '%s/schools.shp' % TESTDATA
            ...
            fid = open('%s/quake_schools_percentage_loss_and_mmi.txt'
                       % TESTDATA)
            reference_points = []
            MMI = []
            DAM = []
    ...
    def test_tephra_load_impact(self):
        ...
        hazard_filename = os.path.join(TESTDATA,
                                       'ashload.asc')
        exposure_filename = os.path.join(TESTDATA, 'schools.shp')
    ...
    def test_interpolation_lembang(self):
        ...
        # Name file names for hazard level, exposure and expected fatalities
        hazard_filename = '%s/hazmap.asc' % TESTDATA
        exposure_filename = '%s/schools.shp' % TESTDATA
    ...
    def test_interpolation_tsunami_maumere(self):
        ...
        # Name file names for hazard level, exposure and expected fatalities
        hazard_filename = ('%s/tsunami_depth2.asc'
                           % TESTDATA)
        exposure_filename = ('%s/tsunami_exposure2.shp' % TESTDATA)
    ...
    def Xtest_padang_building_examples(self):
        ...
        for mmi_filename in ['shakemap.asc']:
            ...
            exposure_filename = os.path.join(TESTDATA, 'buildings.shp')
